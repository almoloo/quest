...
contract Quest is ERC1155, Ownable {
    ...
    function setProfile(
        ...
        string memory _links) external {
        profiles[msg.sender] = UserProfile(_name, _email, _bio, _avatarUrl, _coverUrl, _links);
    ...
    */
    function getNftsByCreator(address creator) external view returns (NftData[] memory) {
        uint256 count = 0;
     ...
     */
    function editeAchivment(uint256 id, string memory url, bool transferable) public {
        require(nftData[id].creator == msg.sender, "Only the creator can edit the achievement");
     ...
     */
    function sendAchivment(address account, uint256 achievementId, uint256 amount, bytes memory data)
        public
    {
        require(nftData[achievementId].creator == msg.sender, "Only the creator can send the achievement");
     ...
     */
    function sendAchivmentBatch(address to, uint256[] memory achievementIds, uint256[] memory amounts, bytes memory data)
        public
    {
        require(achievementIds.length == amounts.length, "IDs and amounts length mismatch");
        for (uint256 i = 0; i < achievementIds.length; i++) 
        {
            require(nftData[achievementIds[i]].creator == msg.sender, "Only the creator can send the achievement");
        } 
        _mintBatch(to, achievementIds, amounts, data);
    }

    /**
     * @notice Sends an achievement to multiple recipients.
     */
    function sendAchivmentmultiRecipient(address[] memory to, uint256 achievementId, uint256[] memory amounts, bytes memory data) external onlyOwner {
        require(to.length == amounts.length, "to and amounts length mismatch");

        for (uint256 i = 0; i < to.length; i++) {
            _mint(to[i], achievementId, amounts[i], data);
        }
    }
    ...
    function safeTransferFrom(address from, address to, uint256 id, uint256 amount, bytes memory data) public override {
        require(nftData[id].transferable, "Token is frozen and cannot be transferred");
    ...
    function safeBatchTransferFrom(address from, address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) public override {
        for (uint256 i = 0; i < ids.length; i++) {
            require(!nftData[ids[i]].transferable, "One or more tokens are frozen and cannot be transferred");
        }
